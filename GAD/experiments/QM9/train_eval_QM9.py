...
def evaluate_network(model, data_loader, prop_idx, factor, device, diffusion_operator):
    ...
    # Initialize a list to store individual property MAEs, if prop_idx is a list
    if isinstance(prop_idx, list):
        num_properties = len(prop_idx)
        individual_epoch_test_maes = torch.zeros(num_properties).to(device)
    else:
        individual_epoch_test_maes = torch.zeros(1).to(device)  # For single property case
        num_properties = 1

    epoch_test_mae = 0  # For combined MAE

    # Use reduction='none' to get per-element loss for calculating individual MAEs
    ...
    with torch.no_grad():
        for idx, batched_graph in tqdm(enumerate(data_loader)):
            ...
            if isinstance(prop_idx, list):
                ...
                target_properties = batched_graph.y[:, prop_idx].unsqueeze(1).to(device)   # Unsqueeze to make it 2D (batch_size, 1)


            # Calculate loss for each property
            # The loss will be of shape (batch_size, num_properties)
            per_sample_per_property_loss = loss_fn(out_model, target_properties)

            # Calculate combined MAE by averaging over all elements
            combined_loss = per_sample_per_property_loss.mean()
            epoch_test_mae += combined_loss.item()

            # Calculate individual MAEs by averaging over the batch dimension (dim=0)
            individual_batch_maes = per_sample_per_property_loss.mean(dim=0)
            individual_epoch_test_maes += individual_batch_maes

            # loss = loss_fn(out_model, target_properties)
            #
